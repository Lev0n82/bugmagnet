...
describe('processMenuObject', function () {
	...
	it('creates simple menu items out of string-value properties, in order of appearance', function () {
	   ...
	   expect(menuBuilder.menuItem.calls.argsFor(0)).toEqual(['First Item', 'rootM', jasmine.any(Function), 'VAT']);
	   expect(menuBuilder.menuItem.calls.argsFor(1)).toEqual(['Second Item', 'rootM', jasmine.any(Function), 'Corporate Tax']);
	   expect(menuBuilder.menuItem.calls.argsFor(2)).toEqual(['Another Item', 'rootM', jasmine.any(Function), 'Euro VAT']);
	});
	it('creates simple menu items out of objects with _type property, passing the object into the menu as value', function () {
	   ...
	   expect(menuBuilder.menuItem.calls.argsFor(0)).toEqual(['First Item', 'rootM', jasmine.any(Function), {'_type': 'taxtype', 'amount': '200'}]);
	});
	it('creates sub-menus out of string array items, using name as label, in array index order', function () {
		...
		expect(menuBuilder.menuItem.calls.count()).toBe(3);
	   expect(menuBuilder.menuItem.calls.argsFor(0)).toEqual(['VAT', 'subM', jasmine.any(Function), 'VAT']);
	   expect(menuBuilder.menuItem.calls.argsFor(1)).toEqual(['Corporate Tax', 'subM', jasmine.any(Function), 'Corporate Tax']);
	   expect(menuBuilder.menuItem.calls.argsFor(2)).toEqual(['Euro VAT', 'subM', jasmine.any(Function), 'Euro VAT']);
	});
	it('creates sub-menus out of hash items', function () {
		...
		expect(menuBuilder.subMenu).toHaveBeenCalledWith('Taxes', 'rootM');
		expect(menuBuilder.menuItem.calls.count()).toBe(3);
	   expect(menuBuilder.menuItem.calls.argsFor(0)).toEqual(['First Item', 'subM', jasmine.any(Function), 'VAT']);
	   expect(menuBuilder.menuItem.calls.argsFor(1)).toEqual(['Second Item', 'subM', jasmine.any(Function), 'Corporate Tax']);
	   expect(menuBuilder.menuItem.calls.argsFor(2)).toEqual(['Another Item', 'subM', jasmine.any(Function), 'Euro VAT']);
	});
});
